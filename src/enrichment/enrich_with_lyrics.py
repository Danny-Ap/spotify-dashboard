#!/usr/bin/env python3
"""
Lyrics & Language Enrichment Script
Processes songs with has_lyrics: null, fetches lyrics using Genius API,
detects languages using character detection and langdetect, and resolves conflicts.

Processing Flow:
1. Find unprocessed songs (has_lyrics: null)
2. For each song: fetch lyrics → update database → detect language → update database
3. Resolve artist language conflicts based on majority language
4. Handle soundtrack classification
...
# Top classical composers (exact name matches only)
CLASSICAL_COMPOSERS = {
...
class LyricsEnricher:
    def __init__(self, db):
        self.db = db
        self.genius = self.initialize_genius_api()
        self.detector = LanguageDetector()

    def initialize_genius_api(self):
    ...
    def get_unprocessed_songs(self):
        """Get songs with has_lyrics: null (unprocessed songs)"""
        try:
            collection = self.db[SONGS_MASTER_COLLECTION]
    ...
    def fetch_lyrics(self, song_name: str, artist_name: str) -> Tuple[Optional[str], bool]:
        """
        ...
        try:
            ...
            logger.error(f"Error fetching lyrics for '{song_name}' by '{artist_name}': {e}")
            return None, False

    def detect_song_language(self, song_data, lyrics: Optional[str]) -> Tuple[str, str]:
        """
        ...
        if self.detector.detect_hebrew_chars(combined_text):
            return "Hebrew", "character_detection"
        
        # Step 3: Check for Japanese characters in song/artist name
        if self.detector.detect_japanese_chars(combined_text):
            return "Japanese", "character_detection"
        
        # Step 4: Try lyrics if available
        if lyrics and len(lyrics.strip()) >= MIN_LYRICS_LENGTH:
            lang, confidence = self.detector.detect_language_ml(lyrics)
            if lang and confidence >= CONFIDENCE_THRESHOLD:
                return self.detector.normalize_language_code(lang), "lyrics"
        
        # Step 5: Try song title
        if song_name:
            lang, confidence = self.detector.detect_language_ml(song_name)
            if lang and confidence >= CONFIDENCE_THRESHOLD:
                return self.detector.normalize_language_code(lang), "title"
        
        # Step 6: Try artist name
        if artist_name:
            lang, confidence = self.detector.detect_language_ml(artist_name)
            if lang and confidence >= CONFIDENCE_THRESHOLD:
                return self.detector.normalize_language_code(lang), "artist_name"
        
        return "Unknown", "unknown"

    def classify_soundtrack(self, song_data) -> bool:
    ...
    def update_song_in_db(self, song_id, lyrics: Optional[str], has_lyrics: bool, 
    ...
    def resolve_artist_language_conflicts(self):
        """Resolve artist language conflicts based on majority language from their songs"""
        try:
            songs_collection = self.db[SONGS_MASTER_COLLECTION]
            artists_collection = self.db[ARTISTS_MASTER_COLLECTION]
            
            logger.info("🎭 Resolving artist language conflicts...")
            
            # Get language counts per artist from songs
            pipeline = [
                {"$match": {"language": {"$ne": None, "$ne": "Unknown", "$ne": "Soundtrack"}}},
                {"$group": {
                    "_id": {
            ...
            artist_language_stats = list(songs_collection.aggregate(pipeline))
            
            updated_artists = 0
            
            for artist_stat in artist_language_stats:
                artist_name = artist_stat['_id']
                languages = artist_stat['languages']
                
                # Find the language with most songs
                most_common_language = max(languages, key=lambda x: x['count'])
                majority_language = most_common_language['language']
                
                # Check current artist language before updating
                current_artist = artists_collection.find_one({"artist_name": artist_name})
                current_language = current_artist.get('language') if current_artist else None
                
                # Only update if language is different
                if current_language != majority_language:
                    ...
                    if result.modified_count > 0:
                        updated_artists += 1
                        logger.info(f"  ✅ {artist_name}: {current_language} → {majority_language} ({most_common_language['count']} songs)")
            
            logger.info(f"✅ Updated {updated_artists} artist language assignments")
            
        except Exception as e:
            logger.error(f"Error resolving artist language conflicts: {e}")

    def fix_soundtrack_language_issues(self):
...
def main():
    ...
    logger.info("📝 Starting Lyrics & Language Enrichment")
    ...
    try:
        ...
        enricher = LyricsEnricher(db)
        
        # Get unprocessed songs
        unprocessed_songs = enricher.get_unprocessed_songs()
        ...
        for i, song in enumerate(unprocessed_songs, 1):
            ...
            is_soundtrack = enricher.classify_soundtrack(song)
            if is_soundtrack:
                soundtrack_count += 1
            ...
            if not is_soundtrack:
                lyrics, has_lyrics = enricher.fetch_lyrics(song_name, artist_name)
                if has_lyrics:
                    lyrics_found_count += 1
                    logger.info(f"  ✅ Found lyrics ({len(lyrics)} characters)")
                else:
                    logger.info(f"  ❌ No lyrics found")
            ...
            language, detection_method = enricher.detect_song_language(song_with_soundtrack, lyrics)
            logger.info(f"  🌍 Language detected: {language} (method: {detection_method})")
            
            # Step 4: Update database
            success = enricher.update_song_in_db(
                song_id, lyrics, has_lyrics, 
        ...
        # Step 5: Resolve artist language conflicts
        enricher.resolve_artist_language_conflicts()
        
        # Step 6: Fix any remaining soundtrack language issues
        enricher.fix_soundtrack_language_issues()
        
        # Summary
        logger.info("=" * 60)
        logger.info("LYRICS ENRICHMENT SUMMARY")
        logger.info("=" * 60)
        ...
        logger.info(f"Artist language conflicts resolved")
